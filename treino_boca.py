"""
Treina a CNN de BOCEJO no dataset da boca (bocejando/normal) e avalia.

Pre-requisito: rodar 'python coletar_boca.py' antes, para gerar dataset_boca/.
Rode:  python treino_boca.py
"""
...
def treinar(modelo, tr, val, nome):
    ...
    for ep in range(EPOCAS):
        ...
        print(f"  [{nome}] epoca {ep+1:>2}/{EPOCAS}  loss={soma/len(tr):.3f}  "
...
def main():
    ...
    print("=== Treinando CNN de bocejo ===")
    modelo = CNN()
    treinar(modelo, tr, val, "CNN-bocejo")
    ms = velocidade_ms(modelo)
    params = n_params(modelo)
    torch.save(modelo.state_dict(), "cnn_bocejo.pt")

    conf = metricas.matriz_confusao(modelo, val, len(classes), DEVICE)
    texto = metricas.salvar("resultados_boca.txt", "CNN de Bocejo",
                            classes, conf, params, ms, counts)
    print("\n" + texto)
    print("\nModelo salvo (cnn_bocejo.pt) e metricas em resultados_boca.txt")
